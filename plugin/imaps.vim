...
if half <=16
	exe a:lastline
	" put a string below
	exe "norm! o"cmmt"<<-\<esc>o\<esc>\!\!date\<esc>kJxA-"cmmt1
	...
	exe a:firstline
	exe "norm! O"cmmt">>---Modified by ShiliangShan---"cmmt1
	"reture cursor to the new lastline
	exe a:lastline+2
else
	exe a:lastline
	" put a string below
	exe "norm! o"cmmt"<<\<esc>".(half - 14-2)."a-\<esc>o\<esc>\!\!date\<esc>kJx".(half - 14-2)."A-\<esc>A"cmmt1
	" and above. its necessary to put the string below the block of lines
	" first because that way the first line number doesnt change...
	exe a:firstline
	exe "norm! O"cmmt">>\<esc>".(half - 12-2)."a-\<esc>AModified by ShiliangShan\<esc>".(half - 12-2)."a-\<esc>A"cmmt1
	"reture cursor to the new lastline
	exe a:lastline+2
endif
endfunction
